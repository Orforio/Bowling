...
class Game {
	...
	function __construct() {
		...
		for ($i=1; $i <= $this->numberPlayers; $i++) {
			...
			$this->players[] = new Player($this->cli->getUserInput());
		}
	...
	public function inputScore($currentPlayer, $currentFrame) {
		...
		echo "-= = = = =-" . "\n";
		echo "[FRAME: " . $currentFrame . "] - [PLAYER: " . $this->players[$currentPlayer-1]->getName() . "]" . "\n";
		
		do {
			echo $this->players[$currentPlayer-1]->getName() . ": please enter your score for the first ball." . "\n";
		...
		if ($inputBall1Score < 10 || $frame10BonusBall) {	// If the user did not bowl a strike, or if this is the last frame and the user bowled a strike on their first ball, we must process their second ball
			do {
				echo $this->players[$currentPlayer-1]->getName() . ": please enter your score for the second ball." . "\n";
		...
		if ($frame10BonusBall) {	// If this is the last frame and the user got a bonus third ball, we process it now
			do {
				echo $this->players[$currentPlayer-1]->getName() . ": please enter your score for the third ball." . "\n";
		...
		$this->players[$currentPlayer-1]->addScore($inputBall1Score, $inputBall2Score);	// Write the score to the user's scores array
		
		if ($frame10BonusBall) {
			$this->players[$currentPlayer-1]->addScore($inputBall3Score, 0);	// A bonus ball in the last frame is added as frame 11
		}
	...
	public function printScores($currentFrame) {
		...
		for ($printingPlayer = 1; $printingPlayer <= $this->numberPlayers; $printingPlayer++) {	// Cycle through the players in turn
			$playerTotal = 0;
			$playerScores = $this->players[$printingPlayer-1]->getScores();
			
			echo "[PLAYER " . $printingPlayer . ": " . $this->players[$printingPlayer-1]->getName() . "]" . "\n";
			echo "|";
			...
			for ($printingFrame = 1; $printingFrame < $currentFrame; $printingFrame++) {	// Print the first line of scores, the actual pins down on each shot
				if ($printingFrame == 10) {	// Special display rules for frame 10
					...
					if ($playerScores[$printingFrame-1]["ball1"] == 10) {	// Strike in first ball means there will be a bonus ball
						echo " X ";
						$bonusBall = true;
					}
					else {
					...
					if ($playerScores[$printingFrame-1]["ball2"] == 10) {
						echo "X ";
					}
					elseif (($playerScores[$printingFrame-1]["ball1"] + $playerScores[$printingFrame-1]["ball2"]) == 10) {	// Spare on the second ball means there will be a bonus ball
						echo "/ ";
						$bonusBall = true;
					}
					else {
						echo $playerScores[$printingFrame-1]["ball2"] . " ";
					}
					
					if ($bonusBall) {	// Display the bonus ball
						if ($playerScores[$printingFrame]["ball1"] == 10) {
							echo "X ";
						}
						else {
					...
					if ($playerScores[$printingFrame-1]["ball1"] == 10) {	// If score is a strike, display an X instead
						echo "  X  " . "|";
					}
					elseif (($playerScores[$printingFrame-1]["ball1"] + $playerScores[$printingFrame-1]["ball2"]) == 10) {	// If score is a spare, display first ball followed by a slash
						echo " " . $playerScores[$printingFrame-1]["ball1"] . " / " . "|";
					}
					else {	// Not a special score, display normally
						echo " " . $playerScores[$printingFrame-1]["ball1"] . " " . $playerScores[$printingFrame-1]["ball2"] . " " . "|";
					}
				}
			}
			...
			for ($printingFrame = 1; $printingFrame < $currentFrame; $printingFrame++) {	// Prints the second line of scores, the frame totals
				$playerTotal = $playerTotal + $playerScores[$printingFrame-1]["total"];
				if ($printingFrame == 10) {
					echo " " . $playerScores[$printingFrame-1]["total"];
					if ($playerScores[$printingFrame-1]["total"] >= 10) {	// Determines how many spaces to print to keep things lined up
						echo "    ";
					}
					else {
						echo "     ";
	...
	private function calculateScores($currentFrame) {
		for ($calculatingPlayer = 1; $calculatingPlayer <= $this->numberPlayers; $calculatingPlayer++) {	// Cycle through each player in turn
			$playerScores = $this->players[$calculatingPlayer-1]->getScores();
			...
			for ($calculatingFrame = 1; $calculatingFrame < $currentFrame; $calculatingFrame++) {	// Cycle through each frame in turn
				if ($calculatingFrame == 10) {	// Special rules for frame 10
					if ($playerScores[$calculatingFrame-1]["ball1"] == 10) {	// If player got a strike on the first ball of frame 10...
						$currentTally = 10 + $playerScores[$calculatingFrame-1]["ball2"] + $playerScores[$calculatingFrame]["ball1"];	// Then they get a bonus for the next two balls
						$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
						// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";
					}
					elseif (($playerScores[$calculatingFrame-1]["ball1"] + $playerScores[$calculatingFrame-1]["ball2"]) == 10) {	// If player got a spare on the second ball of frame 10...
						$currentTally = 10 + $playerScores[$calculatingFrame]["ball1"];	// Then they get a bonus for the next ball
						$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
						// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";

					}
					else {	// Player got neither a strike nor a spare on frame 10
						$currentTally = $playerScores[$calculatingFrame-1]["ball1"] + $playerScores[$calculatingFrame-1]["ball2"];	// User gets normal scoring
						$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
						// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";

					}
				}
				elseif ($playerScores[$calculatingFrame-1]["ball1"] == 10) {	// Player bowled a strike
					if ($playerScores[$calculatingFrame]["ball1"] == 10) {	// If next frame's ball was also a strike...
						$currentTally = 20 + $playerScores[$calculatingFrame+1]["ball1"];	// Add the ball from the next frame along
						$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
						// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";

					}
					else {	// Next frame's first ball was not a strike...
						$currentTally = 10 + $playerScores[$calculatingFrame]["ball1"] + $playerScores[$calculatingFrame]["ball2"];	// ... so just add the next frame's balls normally
						$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
						// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";
					}
				}
				elseif (($playerScores[$calculatingFrame-1]["ball1"] + $playerScores[$calculatingFrame-1]["ball2"]) == 10) {	// If player bowled a spare...
					$currentTally = 10 + $playerScores[$calculatingFrame]["ball1"];	// Then add next frame's first ball
					$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
					// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";

				}
				else {	// Player bowled neither a strike nor a spare, nor is it frame 10
					$currentTally = $playerScores[$calculatingFrame-1]["ball1"] + $playerScores[$calculatingFrame-1]["ball2"];	// Add this frame's balls, normal scoring
					$this->players[$calculatingPlayer-1]->addTally($calculatingFrame, $currentTally);
					// TESTING // echo "Tally for " . $this->players[$calculatingPlayer-1]->getName() . ", frame " . $calculatingFrame . " is " . $currentTally . "\n";
